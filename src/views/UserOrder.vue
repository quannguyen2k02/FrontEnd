                ...
                <div class="container mt-5  ">
                    <form class="form-add" @submit.prevent="order">
                        <div class="form-group mb-3">
                            ...
                            <input disabled type="text" id="productName" class="form-control" v-model="CustomerName"
                                required />
                        </div>
                        <div class="form-group mb-3">
                            <label for="productName" class="form-label">Số điện thoại:</label>
                            <input disabled type="text" id="productDetail" class="form-control" v-model="PhoneNumber"
                                required />
                        </div>
                        <div class="form-group mb-3">
                            <label for="productDetail" class="form-label">Địa chỉ</label>
                            <input disabled id="productDescription" class="form-control" v-model="Address" required>
                        </div>
                        <div class="form-group mb-3">
                                ...
                                <table class="product-table">
                                    ...
                                    <tbody>
                                        <tr v-for="(product, index) in productList" :key="index">
                                            <td>{{ product.code }}</td>
                                            <td>{{ product.name }}</td>
                                            <td><img style="height: 100px;" :src="product.image" alt="Sản phẩm 1"></td>
                            ...
                            <label for="productDetail" class="form-label">Phương thức thanh toán:{{
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select disabled id="productCategory" class="form-control" v-model="status" required>
                                <option value=0>
                                    Chưa xác nhận
                                </option>
                                <option value=1>
                        ...
                        <button :disabled="status === 2 || status === 3 || status === 4" type="submit"
                            class="btn btn-primary btn-block">Hủy đơn hàng</button>
                    </form>
                </div>
            </div>
        </div>
...
async function getOrder() {
    try {
        const response = await fetch(`https://localhost:7061/api/Orders/${orderId}`);
...
async function getOrderDetails() {
    try {
        const response = await fetch(`https://localhost:7061/api/OrderDetails/orderdetails/${orderId}`);
        const data = await response.json();
...
const payment = (paymentMethod) => {
    switch (paymentMethod) {
        ...
        case 1: return "Đã thanh toán qua ngân hàng";
...
const cancelOrder = async (orderId) => {
    try {
        const response = await axios.post(`https://localhost:7061/api/Orders/${orderId}/cancel`);
